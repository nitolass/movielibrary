    ...
    <div class="container mx-auto px-4 py-10 text-white max-w-4xl">
        ...
        <div class="bg-[#0f1115] rounded-3xl overflow-hidden shadow-2xl border border-white/5 flex flex-col md:flex-row">
            ...
            <div class="w-full md:w-1/3 bg-black/50">
                ...
                @else
                    <div class="w-full h-full flex items-center justify-center bg-gray-800 text-gray-500">Sin Imagen</div>
                @endif
            </div>
            ...
            <div class="w-full md:w-2/3 p-8 space-y-4">
                ...
                <div class="pt-4 space-y-2 border-t border-white/10">
                    <p class="text-sm"><strong class="text-yellow-400 uppercase text-xs">Director:</strong>
                        {{ $movie->director ? $movie->director->name : 'Sin director asignado' }}
                    </p>

                    <p class="text-sm"><strong class="text-yellow-400 uppercase text-xs">Géneros:</strong>
                        {{ $movie->genres->pluck('name')->join(', ') }}
                    </p>

                    <p class="text-sm"><strong class="text-yellow-400 uppercase text-xs">Actores:</strong>
                        {{ $movie->actors->pluck('name')->join(', ') }}
                    </p>
                </div>

                <div class="pt-6">
                    <a href="{{ route('movies.index') }}" class="px-5 py-2 bg-gray-700 hover:bg-gray-600 rounded-xl font-bold transition-colors">← Volver</a>
                    <a href="{{ route('movies.edit', $movie) }}" class="ml-2 px-5 py-2 bg-yellow-400 text-black hover:bg-yellow-300 rounded-xl font-bold transition-colors">Editar</a>
                </div>
            </div>
        </div>
    </div>
