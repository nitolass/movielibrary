    ...
    <div class="container mx-auto px-6 py-8" x-data="{ loaded: false }" x-init="setTimeout(() => loaded = true, 100)">
        ...
        <div class="flex flex-col md:flex-row justify-between items-end mb-10 gap-6 transition-all duration-700 opacity-0 translate-y-4" :class="loaded ? 'opacity-100 translate-y-0' : ''">
            ...
            <div class="flex items-center gap-4">
                ...
                <select class="bg-gray-800/50 backdrop-blur-md text-gray-300 text-sm font-bold border border-white/10 rounded-xl py-2.5 px-4 hover:bg-white/10 hover:border-white/30 hover:text-white transition-all cursor-pointer focus:ring-2 focus:ring-yellow-400 focus:border-transparent appearance-none pr-8 relative" style="background-image: url('data:image/svg+xml;charset=utf-8,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' fill=\'none\' viewBox=\'0 0 20 20\'%3E%3Cpath stroke=\'%239CA3AF\' stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'1.5\' d=\'M6 8l4 4 4-4\'/%3E%3C/svg%3E'); background-position: right 0.5rem center; background-repeat: no-repeat; background-size: 1.5em 1.5em;">
            ...
            @forelse($movies as $movie)
                <div class="group flex flex-col h-full relative">
                    ...
                    <div class="relative w-full aspect-[2/3] rounded-2xl overflow-hidden bg-gray-900/50 border border-white/5 shadow-lg group-hover:shadow-[0_0_40px_rgba(0,0,0,0.6)] group-hover:border-yellow-500/50 transition-all duration-500 mb-4 z-0 group-hover:z-10">
                        ...
                        @if($movie->image)
                            <img src="{{ asset('storage/' . $movie->image) }}" alt="{{ $movie->title }}" class="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-700 ease-in-out will-change-transform">
                        @else
                        ...
                        <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-center items-center gap-3 p-6 backdrop-blur-[2px] z-20">
                            ...
                            <div class="flex gap-2 w-full">
                                <a href="{{ route('movies.edit', $movie) }}" class="flex-1 py-2.5 bg-gray-800/80 backdrop-blur-md text-white text-xs font-bold rounded-lg hover:bg-gray-700 transition-all text-center border border-white/10 hover:border-white/30 hover:text-yellow-200">
                                    Editar
                                </a>
                                <form action="{{ route('movies.destroy', $movie) }}" method="POST" onsubmit="return confirm('¿Estás seguro de querer eliminar &quot;{{ $movie->title }}&quot;?');" class="flex-1">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-full py-2.5 bg-red-600/80 backdrop-blur-md text-white text-xs font-bold rounded-lg hover:bg-red-500 transition-all text-center border border-transparent hover:border-red-400/50 hover:shadow-[0_0_15px_rgba(239,68,68,0.4)]">
                                        Borrar
                                    </button>
                                </form>
                            </div>
