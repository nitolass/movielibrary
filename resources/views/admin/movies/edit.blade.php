...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold text-white font-['Outfit']">Editar <span class="text-yellow-400">Película</span></h1>
        ...
        <div class="bg-[#0f1115] border border-white/5 rounded-xl p-8 shadow-xl max-w-3xl mx-auto">
            <form action="{{ route('movies.update', $movie) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <x-select name="director_id" required>
                            ...
                            @foreach($directors as $director)
                                <option value="{{ $director->id }}"
                                    {{-- Comprobamos si es el director guardado o el que seleccionó antes de un error --}}
                        ...
                        @foreach($genres as $genre)
                            <label class="flex items-center space-x-3 p-3 bg-[#16181c] border border-gray-700 rounded-xl hover:border-yellow-400/50 cursor-pointer transition-colors">
                                <input type="checkbox" name="genres[]" value="{{ $genre->id }}"
                                    ...
                                    {{-- Lógica potente: Check si está en el old (error form) O si la película ya tiene ese género --}}
                ...
                {{-- Poster --}}
                <div>
                    <x-label for="poster" value="Cambiar Póster (Opcional)" />

                    @if($movie->poster)
                        <div class="mb-3">
                            <p class="text-xs text-gray-500 mb-1">Póster Actual:</p>
