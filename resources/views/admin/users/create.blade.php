...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold text-white font-['Outfit']">{{ __('Crear') }} <span class="text-yellow-400">{{ __('Usuario') }}</span></h1>
            <a href="{{ route('admin.users.index') }}" class="text-gray-400 hover:text-white transition-colors">&larr; {{ __('Volver') }}</a>
        </div>
        ...
        <div class="bg-[#0f1115] border border-white/5 rounded-xl p-8 shadow-xl max-w-2xl mx-auto">
            <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <x-label for="role_id" :value="__('Rol de Usuario')" />
                ...
                <div class="flex justify-end gap-4 pt-4 border-t border-white/5">
                    <a href="{{ route('admin.users.index') }}" class="px-6 py-3 bg-gray-800 text-gray-300 font-bold rounded-xl hover:bg-gray-700 transition-colors">{{ __('Cancelar') }}</a>
