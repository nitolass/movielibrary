...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold text-white font-['Outfit']">{{ __('Nuevo') }} <span class="text-yellow-400">{{ __('Género') }}</span></h1>
        ...
        <div class="bg-[#0f1115] border border-white/5 rounded-xl p-8 shadow-xl max-w-xl mx-auto">
            <form action="{{ route('admin.genres.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    {{-- Placeholder también traducido --}}
                    <x-input id="name" type="text" name="name" :value="old('name')" required placeholder="{{ __('Ej: Ciencia Ficción') }}" />
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>

                <div>
                    {{-- Simplificamos a 'Descripción' para usar la llave del JSON --}}
