...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-white font-['Outfit']">Gestión de <span class="text-yellow-400">Géneros</span></h1>
        ...
        <div class="bg-[#0f1115] border border-white/5 rounded-xl overflow-hidden shadow-xl">
                ...
                @foreach($genres as $genre)
                    <tr class="border-b border-white/5 hover:bg-white/5 transition-colors">
                        ...
                        <td class="py-4 px-6 flex justify-center gap-2">
                            {{-- Botón Editar --}}
                            <a href="{{ route('admin.genres.edit', $genre) }}" class="p-2 bg-gray-800 text-blue-400 rounded-lg hover:bg-blue-600 hover:text-white transition-all" title="Editar">
                            ...
                            {{-- Botón Eliminar --}}
                            <form action="{{ route('admin.genres.destroy', $genre) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar el género {{ $genre->name }}?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2 bg-gray-800 text-red-400 rounded-lg hover:bg-red-600 hover:text-white transition-all" title="Eliminar">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @if($genres->isEmpty())
                <div class="p-8 text-center text-gray-500">
                    <p>No hay géneros registrados aún.</p>
                </div>
            @endif
        </div>
    </div>
