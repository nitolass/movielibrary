    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-6">
            <a href="{{ route('user.movies.index') }}" class="inline-flex items-center gap-2 text-gray-400 hover:text-yellow-400 transition-colors text-sm font-medium group">
                ...
                <span>{{ __('Volver al Catálogo') }}</span>
            </a>
        </div>

        {{-- CABECERA --}}
        <div class="flex items-center gap-3 mb-8">
            <h1 class="text-3xl font-black text-white font-['Outfit']">{{ __('Mis Reseñas') }} <span class="text-yellow-400">.</span></h1>
            <span class="text-gray-500 text-sm mt-2">{{ __('Gestiona tus opiniones aquí.') }}</span>
        </div>

        {{-- MENSAJE DE ÉXITO --}}
        @if (session()->has('message'))
            <div class="bg-green-500/10 border border-green-500 text-green-400 p-4 rounded-xl mb-6 flex items-center gap-2">
                <span>✅</span> {{ __(session('message')) }}
            </div>
        @endif
        ...
        @if($isEditing)
            <div class="bg-[#16181c] border border-yellow-500/50 p-6 rounded-2xl mb-8 shadow-[0_0_20px_rgba(250,204,21,0.1)] max-w-2xl mx-auto">
                <h3 class="text-xl font-bold text-yellow-400 mb-4">{{ __('Editar Reseña') }}</h3>
                <div class="space-y-4">
                    <div>
                    ...
                    <div class="flex justify-end gap-3">
                        <button wire:click="cancel" class="px-4 py-2 bg-gray-700 text-white rounded-xl hover:bg-gray-600 transition-colors">{{ __('Cancelar') }}</button>
                        <button wire:click="update" class="px-4 py-2 bg-yellow-400 text-black font-bold rounded-xl hover:bg-yellow-500 transition-colors shadow-lg shadow-yellow-400/20">{{ __('Guardar') }}</button>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-[#16181c] border border-white/5 p-5 rounded-2xl hover:border-yellow-400/30 transition duration-300 flex flex-col h-full group hover:-translate-y-1 hover:shadow-xl">
                    ...
                    <div class="flex gap-4 mb-4">
                        <div class="shrink-0">
                            ...
                            @else
                                <div class="w-16 h-24 bg-gray-800 rounded-lg flex items-center justify-center text-xs text-gray-500">{{ __('Sin img') }}</div>
                            @endif
                        </div>
                        <div>
                            <h3 class="font-bold text-lg text-white group-hover:text-yellow-400 transition line-clamp-2">
                                {{ $review->movie->title ?? __('Película borrada') }}
                            </h3>
                    ...
                    <div class="flex justify-end gap-3 mt-auto pt-3 border-t border-white/5">
                        ...
                        <button wire:click="delete({{ $review->id }})"
                                wire:confirm="{{ __('¿Estás seguro de que quieres borrar esta reseña?') }}"
            ...
            @empty
                <div class="col-span-full py-20 text-center border border-dashed border-gray-700 rounded-3xl bg-[#16181c]/50">
                    <p class="text-gray-400 text-lg mb-4">{{ __('Aún no has escrito ninguna reseña.') }}</p>
