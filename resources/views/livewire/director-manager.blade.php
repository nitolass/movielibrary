<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-white font-['Outfit']">{{ __('Gestión de') }} <span class="text-yellow-400">{{ __('Directores') }}</span></h1>

        <div class="flex gap-4">
            <x-input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ __('Buscar director...') }}" class="w-64" />
        ...
        <table class="min-w-full leading-normal text-left">
            ...
            @foreach($directors as $director)
                <tr class="border-b border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td class="py-4 px-6">
                        @if($director->photo)
                            <img src="{{ asset('storage/' . $director->photo) }}" class="h-10 w-10 rounded-full object-cover border border-gray-600">
                        @else
                    ...
                    <td class="py-4 px-6 flex justify-center gap-2">
                        <button wire:click="edit({{ $director->id }})" class="p-2 bg-gray-800 text-blue-400 rounded-lg hover:bg-blue-600 hover:text-white transition-all">
                            {{ __('Editar') }}
                        </button>
                        <button wire:click="delete({{ $director->id }})" wire:confirm="{{ __('¿Estás seguro de eliminar?') }} {{ $director->name }}?" class="p-2 bg-gray-800 text-red-400 rounded-lg hover:bg-red-600 hover:text-white transition-all">
                            {{ __('Eliminar') }}
                        </button>
                    </td>
                </tr>
            ...
            <div class="bg-[#1a1d24] border border-white/10 text-white rounded-xl shadow-2xl w-full max-w-2xl p-8 relative max-h-[90vh] overflow-y-auto">
                ...
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-bold font-['Outfit']">
                        {{ $directorId ? __('Editar') . ' ' . __('Director') : __('Crear') . ' ' . __('Nuevo') . ' ' . __('Director') }}
                    </h2>
                    <button wire:click="closeModal()" class="text-gray-400 hover:text-white">&times;</button>
                </div>

                <form wire:submit.prevent="{{ $directorId ? 'update' : 'store' }}" class="space-y-6">
                    ...
                    <div class="flex justify-end gap-4 pt-4 border-t border-white/5">
                        ...
                        <button type="submit" class="px-4 py-2 bg-yellow-400 text-black font-bold rounded-lg hover:bg-yellow-300 shadow-lg transition">
                            {{ $directorId ? __('Actualizar') : __('Guardar') }}
                        </button>
                    </div>
                </form>
            </div>
