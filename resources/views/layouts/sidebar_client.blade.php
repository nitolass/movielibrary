    ...
    <div class="h-20 flex items-center px-8 border-b border-white/5">
        <a href="{{ route('user.dashboard') }}" class="text-3xl font-black tracking-tighter text-white font-['Outfit']">
    ...
    <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
        ...
        <p class="px-4 text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">{{ __('Menú Principal') }}</p>

        {{-- Inicio --}}
        <a href="{{ route('user.dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('home') ? 'bg-yellow-400 text-black font-bold shadow-[0_0_15px_rgba(250,204,21,0.4)]' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
        ...
        @auth
            <div class="mt-6 pt-6 border-t border-white/5">
                <p class="px-4 text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">{{ __('Biblioteca') }}</p>
                ...
                <a href="{{ route('user.watched') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('user.watched') ? 'bg-yellow-400 text-black font-bold' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                    ...
                    <span>{{ __('Ya vistas') }}</span>
                </a>

                {{-- Reseñas --}}
                <a href="{{ route('user.reviews.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('user.rated') ? 'bg-yellow-400 text-black font-bold' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="mt-6 pt-6 border-t border-white/5">
                <p class="px-4 text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">{{ __('Cuenta') }}</p>
        ...
        @else
            ...
            <div class="mt-6 pt-6 border-t border-white/5 px-4">
                <p class="text-xs text-gray-500 mb-3">{{ __('Únete a la comunidad') }}</p>
