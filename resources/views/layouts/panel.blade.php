...
<div :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-64 transition-transform duration-300 md:translate-x-0 bg-[#0f1115] border-r border-white/5">
    @include('layouts.sidebar')
</div>

<div x-show="sidebarOpen" @click="sidebarOpen = false" class="fixed inset-0 bg-black/50 z-40 md:hidden backdrop-blur-sm"></div>
...
<div class="flex-1 flex flex-col min-h-screen transition-all duration-300 md:ml-64">
    ...
    <header class="h-20 flex items-center justify-between px-6 py-4 bg-[#0f1115]/80 backdrop-blur-md sticky top-0 z-30 border-b border-white/5">
        ...
        <div class="hidden md:flex items-center w-full max-w-md relative mx-4">
            ...
            <input type="text" placeholder="Buscar películas, géneros..." class="w-full bg-[#1e2126] border-transparent focus:border-yellow-400/50 text-gray-200 text-sm rounded-full pl-12 pr-4 py-2.5 focus:ring-0 transition-all placeholder-gray-500">
        </div>
